 ...
 *     - Proxy (or reverse proxy) is created with a dns name, and all the dynamics of starting/restarting
 *     /switching from one host to another – everything is nice and clear for the clients. Infrastructure
 *     does all the hard work out of the box.
 *     - Configure sets the static registry.
 *
 * - Dynamic discovery: every time a service starts, it registers its address in the discovery service ("Service
 * name" at the following address "IP"). Clients then ask to resolve the address by which the requested service
 * can be reached. The service has a general name, by which other services can resolve it.
 *     - If a service stops working, you need to refresh its address, clean stale addresses, heartbeats must be
 *     used – lots of problems and challenges.
 *
